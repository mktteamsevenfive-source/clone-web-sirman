"""
Test DWH API PDF Download with Full Headers
"""
...
async def main():
    async with async_playwright() as p:
        ...
        print(f"  Authorization: {captured_headers.get('authorization', '')[:30]}...")
        print(f"  x-company: {captured_headers.get('x-company')}")
        print(f"  x-customer-code: {captured_headers.get('x-customer-code')}")

        # Clean headers for API request
        api_headers = {k: v for k, v in captured_headers.items() if k.lower() in [
            'authorization', 'x-company', 'x-customer-code', 'x-user-device', 'x-language', 'user-agent'
        ]}
        ...
        for tf in test_files:
            ...
            for u in urls:
                ...
                print(f"[TEST] {u} -> Status: {resp.status}, Content-Type: {resp.headers.get('content-type')}, Size: {len(body)} bytes, StartsWithPDF: {body.startswith(b'%PDF')}")
                if resp.status == 200 and body.startswith(b'%PDF'):
